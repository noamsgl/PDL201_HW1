...
def gradient_test(input_dim, output_dim, jacobian=False, samples=1,
    ...
    for i in range(iters):
        ...
        if jacobian:
            ...
            # TODO - check the line in comment why not good
            eps_delta_grad_x = (eps_delta.reshape(1, -1) @ grad.reshape(-1, 1))[0, 0]
            # eps_delta_grad_x = np.sum(eps_delta.T @ grad)
        ...
        epsilon = 0.5 * epsilon
    title = "Hidden - " if jacobian else ""
    plot_gradient_test(layer, param, ratio_eps, ratio_eps_sqr, iters, test_title=f'{title}{str(net.layers_sizes)}')
...
def plot_gradient_test(layer, param, ratio1_list, ratio2_list, iterations, test_title=""):
    # plot results:
    X_axis = np.arange(iterations)
    fig, axs = plt.subplots(1, 1, figsize=(16, 4))
    fig.suptitle('The Gradient Test', fontsize='x-large', y=1.1)
    axs.set_title(f'{test_title} - Ratio Test for {param}_{layer}')
...
gradient_test_for_all_params(input_dim, output_dim, samples=1, hidden_layers_sizes=hidden_layers_for_tests, iters=15, optimizer='None')
# jacobian test for a hidden layer derivative - when given no layers (None), create one layer and checks it
gradient_test_for_hidden_layer_jacobian(input_dim, output_dim, samples=1, hidden_layers_sizes=None, iters=15, optimizer='None')
# jacobian test for a hidden layer derivative - checks every parameter in the hidden layers
gradient_test_for_hidden_layer_jacobian(input_dim, output_dim, samples=1, hidden_layers_sizes=hidden_layers_for_tests, iters=15, optimizer='None')
